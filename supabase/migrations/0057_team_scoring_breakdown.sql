-- 0057_team_scoring_breakdown.sql
-- What a team averages, and which part of it is doing the work.
--
-- Two numbers people actually want: how many points a week the side is good
-- for, and whether that is coming from the front or the back. The second is the
-- one that changes behaviour — "my defenders average four" is an argument for a
-- transfer in a way that a league table never is.
--
-- Averaged over every completed gameweek in the season, not over the weeks a
-- lineup happened to be set. A week nobody picked a team is a week that scored
-- nothing, and hiding it would flatter the average precisely when it shouldn't.
--
-- The captain's double is attributed to the group the captain plays in, so the
-- four positions sum exactly to the team's total. A defender captained into 20
-- points did put 20 points on the board for the defence.
...
comment on function team_scoring_breakdown(uuid) is
  ...
  'gameweek. The captain''s double counts towards his own position, so the four '
  'positions add up to the team row.';
